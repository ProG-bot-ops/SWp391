...
class PatientAppointmentManager {
    ...
    async loadAppointments() {
        try {
            ...
            const userId = this.getUserInfo()?.id; // Assuming userInfo includes an 'id'
            if (!userId) {
                console.log('⚠️ Không có ID người dùng, chuyển sang dữ liệu mẫu');
                this.loadSampleData();
                return;
            }

            const response = await fetch(`${this.apiBaseUrl}/api/appointment/patient-appointments/test?userId=${userId}`, {
                method: 'GET',
                headers: {
    ...
    setupCalendar() {
        ...
        try {
            this.calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                slotMaxTime: "19:00:00", // slotMaxTime là exclusive, nên phải là 19:00 để hiển thị đến 18:00
                ...
                slotLabelContent: function(arg) {
                    ...
                    if (hour >= 7 && hour < 12) return 'Ca sáng ' + arg.date.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
                    if (hour >= 13 && hour < 18) return 'Ca chiều ' + arg.date.toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'});
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="info-block">
                                        <ul class="list-unstyled mb-0">
                                            <li><strong>Ngày:</strong> ${appointment.appointmentDate}</li>
                                            <li><strong>Giờ:</strong> ${appointment.startTime}${appointment.endTime ? ' - ' + appointment.endTime : ''}</li>
                                            <li><strong>Trạng thái:</strong> <span class="status-badge ${this.getStatusClass(appointment.status)}">${this.getStatusText(appointment.status)}</span></li>
                                            ${appointment.patient && appointment.patient.name ? `<li><strong style='color:#007bff;'>Bệnh nhân:</strong> <span style='font-weight:600;'>${appointment.patient.name}</span></li>` : ''}
                                            ${appointment.service && appointment.service.name ? `<li><strong>Dịch vụ:</strong> ${appointment.service.name}</li>` :
      (appointment.ServiceId && this.servicesMap && this.servicesMap[appointment.ServiceId] ? `<li><strong>Dịch vụ:</strong> ${this.servicesMap[appointment.ServiceId]}</li>` :
      (appointment.serviceId && this.servicesMap && this.servicesMap[appointment.serviceId] ? `<li><strong>Dịch vụ:</strong> ${this.servicesMap[appointment.serviceId]}</li>` :
      (appointment.service && appointment.service.id && this.servicesMap && this.servicesMap[appointment.service.id] ? `<li><strong>Dịch vụ:</strong> ${this.servicesMap[appointment.service.id]}</li>` :
      '')))}
                                            ${appointment.service && appointment.service.price ? `<li><strong>Giá dịch vụ:</strong> ${this.formatCurrency(appointment.service.price)}</li>` : ''}
                                            ${appointment.note ? `<li><strong>Ghi chú:</strong> ${appointment.note}</li>` : ''}
                                        </ul>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                        ...
                                        ${appointment.doctors.map(doctor => `
                                            <div class="doctor-info">
                                                ...
                                                <div>
                                                    ...
                                                    ${doctor.service ? `<div class='doctor-service'><strong>Dịch vụ bác sĩ:</strong> ${doctor.service.name}${doctor.service.price ? ` (${this.formatCurrency(doctor.service.price)})` : ''}</div>` : (doctor.specialty ? `<div class='doctor-service'><strong>Chuyên khoa:</strong> ${doctor.specialty}</div>` : '')}
                                                </div>
                                            </div>
                                        `).join('')}
                                    </div>
    ...
    async fetchAllServices() {
        try {
            const response = await fetch(`${this.apiBaseUrl}/api/Service`);
            if (!response.ok) throw new Error('Không thể lấy danh sách dịch vụ');
            const services = await response.json();
            // Tạo map id -> name
            this.servicesMap = {};
            (services || []).forEach(s => {
                this.servicesMap[s.id] = s.name;
            });
        } catch (e) {
            this.servicesMap = {};
