...
class TimeSlotIntegration {
    ...
    updateTimeSlotDisplay() {
        // Chỉ hiển thị 2 nút ca sáng và ca chiều
        const timeButtons = document.querySelectorAll('.time-btn.shift-btn');
        if (!timeButtons || timeButtons.length < 2) return;

        // Luôn hiển thị 2 nút
        timeButtons.forEach(btn => btn.style.display = '');

        // Nếu có dữ liệu slot, kiểm tra xem ca nào còn slot
        // Nếu không có dữ liệu, mặc định enable cả 2 ca
        let morningAvailable = true;
        let afternoonAvailable = true;
        if (this.availableTimeSlots) {
            if (Array.isArray(this.availableTimeSlots.morning)) {
                morningAvailable = this.availableTimeSlots.morning.some(slot => slot.available === true);
            }
            if (Array.isArray(this.availableTimeSlots.afternoon)) {
                afternoonAvailable = this.availableTimeSlots.afternoon.some(slot => slot.available === true);
            }
        }
        ...
        timeButtons.forEach(btn => {
            ...
            if (shift === 'morning') {
                btn.disabled = !morningAvailable;
                btn.classList.toggle('disabled', !morningAvailable);
                btn.style.opacity = morningAvailable ? '1' : '0.5';
            } else if (shift === 'afternoon') {
                btn.disabled = !afternoonAvailable;
                btn.classList.toggle('disabled', !afternoonAvailable);
                btn.style.opacity = afternoonAvailable ? '1' : '0.5';
            }
        });
    }
